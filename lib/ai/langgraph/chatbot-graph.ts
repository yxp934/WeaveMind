...
import {
  ...
  routeDecisionNode,
} from "./nodes/intent-recognition-node";
import {
  courseCreationNode,
  continueWorkflowNode,
  outlineGenerationNode,
  assignmentCreationNode,
  a2aOptimizationNode,
...
export class LangGraphChatbot {
  ...
  async processMessage(
    ...
    try {
      ...
      // 将本次请求携带的上下文写入state，供节点进行“选中班级/会话/作业”等操作（避免UI选中但后端不知情）
      state = {
        ...state,
        metadata: {
          ...state.metadata,
          requestContext,
          selectedClassId:
            requestContext.selectedClassId || requestContext.classId,
          selectedSessionId: requestContext.selectedSessionId,
          selectedAssignmentId: requestContext.selectedAssignmentId,
          selectedContexts: requestContext.selectedContexts || [],
      ...
      if (conversationHistory.length > 0) {
        ...
        let latestSelectedClassId: string | null = null;
        let latestSelectedSessionId: string | null = null;
        ...
        if (
          ...
          state.intent?.parameters?.currentWorkflowType
        ) {
          state.currentWorkflow = {
        ...
        state.metadata = {
          ...
          selectedSessionId:
            latestSelectedSessionId || state.metadata?.selectedSessionId,
          selectedAssignmentId:
            latestSelectedAssignmentId || state.metadata?.selectedAssignmentId,
          lastCreatedClassId,
          lastCreatedJoinCode,
        };
      }
