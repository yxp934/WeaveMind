...
export function responseGeneratorNode(state: ChatbotState): { messages: any[], response: any } {
  ...
  const structuredResponse = generateStructuredResponse(state, aiContent, additionalKwargs)
...
function generateIntelligentDefaultResponse(state: ChatbotState): any {
  ...
  return {
    ...
    choices: [
      { id: 'create_course', text: '创建新课程', description: '创建一个完整的教学课程' },
      { id: 'generate_outline', text: '生成课程大纲', description: '基于主题生成详细的教学大纲' },
      { id: 'create_assignment', text: '创建作业', description: '设计测验、写作或研究作业' },
      { id: 'a2a_optimization', text: 'A2A内容优化', description: '使用AI优化现有课程内容' },
      { id: 'content_generation', text: '生成教学内容', description: '创建具体的教学材料' }
    ],
    toolsUsed: [],
    metadata: {
      intent: 'general_chat',
      userRole: state.userRole,
      availableActions: ['create_course', 'generate_outline', 'create_assignment', 'a2a_optimization', 'content_generation'],
 ...
 * 生成结构化响应
 */
function generateStructuredResponse(state: ChatbotState, aiContent: string, additionalKwargs: any): any {
  const intent = state.intent?.type || 'unknown'
  const workflowType = state.currentWorkflow?.type
  ...
  if (workflowType === 'course_creation') {
    ...
    choices = [
      { id: 'topic_clarification', text: '明确课程主题', description: '详细说明要创建的课程主题和目标' },
      { id: 'quick_course', text: '快速创建课程', description: '使用默认设置快速创建基础课程' },
      { id: 'from_outline', text: '基于大纲创建', description: '先生成大纲再创建课程' }
    ]
  } else if (intent === 'outline_generation') {
    choices = [
      { id: 'provide_details', text: '提供课程详情', description: '详细描述课程主题、目标受众等' },
      { id: 'quick_outline', text: '快速生成大纲', description: '基于简单描述生成大纲' },
      { id: 'template_based', text: '使用模板', description: '基于现有模板生成大纲' }
    ]
  }

  // 关键修复5: 生成响应消息，包含引导性内容
  let message = aiContent
  if (state.metadata?.suggestedResponse) {
    message = state.metadata.suggestedResponse
  }

  // 如果没有choices但有缺失信息，生成引导性问题
  if (choices.length === 0 && state.metadata?.missingInfo?.length > 0) {
    message += '\n\n' + generateGuidedQuestions(state.metadata.missingInfo)
  }
  ...
  return {
    ...
    metadata: {
      ...
      availableActions: state.metadata?.availableActions || ['continue_workflow', 'restart', 'help'],
 ...
 * 生成工作流相关的选择题
 */
function generateWorkflowChoices(workflowType: string, state: ChatbotState): any[] {
  switch (workflowType) {
    case 'course_creation':
      return generateCourseCreationChoices(state)
    case 'outline_generation':
      return [
        { id: 'provide_details', text: '提供更多详情', description: '补充课程相关信息' },
        { id: 'generate_now', text: '立即生成', description: '基于现有信息生成大纲' },
        { id: 'restart', text: '重新开始', description: '重新开始大纲生成流程' }
      ]
    case 'assignment_creation':
      return [
        { id: 'quiz_assignment', text: '创建测验', description: '设计多项选择题测验' },
        { id: 'writing_assignment', text: '创建写作作业', description: '设计写作类作业' },
        { id: 'research_assignment', text: '创建研究作业', description: '设计研究性作业' }
      ]
    default:
      return [
        { id: 'continue', text: '继续当前流程', description: '继续完成当前工作' },
...
function generateGuidedQuestions(missingInfo: string[]): string {
  ...
  if (missingInfo.includes('course_duration')) {
    questions.push('⏰ **课程时长**：您希望课程持续多长时间？（如：4节、8节、12节）')
  }
  if (missingInfo.includes('sessions_per_week')) {
