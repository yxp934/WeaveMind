 ...
 * 生成详细的课程会话内容
 */
function generateDetailedSessions(
  courseName: string,
  ...
  for (let week = 1; week <= weeks; week++) {
    ...
    const sessionsThisWeek = Math.min(
      courseInfo.sessionsPerWeek || 2,
      totalSessions - (week - 1) * (courseInfo.sessionsPerWeek || 2),
    );

    for (let session = 1; session <= sessionsThisWeek; session++) {
      const sessionNum =
        (week - 1) * (courseInfo.sessionsPerWeek || 2) + session;
...
function getSessionDescription(
  ...
  if (topic.toLowerCase().includes("java")) {
    ...
    return (
      descriptions[sessionNum - 1] ||
      `深入学习${courseName}的相关知识和实践应用`
    );
  } else if (topic.toLowerCase().includes("python")) {
    const descriptions = [
...
function getLearningObjectives(
  ...
  if (topic.toLowerCase().includes("java")) {
    ...
    return (
      objectives[sessionNum - 1] || `掌握${courseName}的核心知识点和实践技能`
    );
  } else if (topic.toLowerCase().includes("python")) {
    const objectives = [
 ...
 */
export async function courseCreationNode(
  state: ChatbotState,
...
- 优先满足用户明确表达的创建课程需求`;
    ...
    if (result.action === "generate_course") {
      ...
      const userAskedAutoSave = /直接|无需确认|马上|立即|立刻/.test(
      ...
      // 生成详细的课程会话内容
      const detailedSessions = generateDetailedSessions(
        courseInfo.topic || "未命名课程",
        totalSessions,
        courseInfo,
      );
...
如果您对任何session的内容有修改建议，请告诉我。确认无误后，我将为您创建这个课程并保存到数据库。`;
      ...
      // **关键修改**：默认不立即保存，除非用户明确要求“直接/立即创建”
      result.metadata = {
        ...result.metadata,
        toolsUsed: ["detailed_course_generation"],
        requiresDatabaseAction: userAskedAutoSave,
        actionType: userAskedAutoSave ? "create_course_with_sessions" : null,
        actionData: userAskedAutoSave
          ? {
      ...
      if (userAskedAutoSave) {
        // 用户要求直接创建：调整提示语，避免“已创建”幻觉，交由DB操作完成
        result.message = `收到，将直接为您创建课程并完成排课（${totalSessions}节，${sessionsPerWeek}节/周）。创建完成后我会返回加入代码。`;
        console.log("⚡ 用户要求直接创建课程，已开启数据库保存流程");
      }
      ...
      const courseInfo =
        ...
        state.courseInfo ||
        state.metadata?.pendingCourseData?.courseInfo ||
        {};
...
正在创建班级和所有课程会话，请稍等...`;
    ...
    if (
      result.updatedCourseInfo &&
      Object.keys(result.updatedCourseInfo).length > 0
    ) {
 ...
 * 修复版本：完善所有六个核心工作流的实现
 */
export async function continueWorkflowNode(
  state: ChatbotState,
): Promise<Partial<ChatbotState>> {
  const lastMessage = state.messages[state.messages.length - 1];
    ...
    switch (state.currentWorkflow?.type) {
      ...
      default:
        ...
        if (inferredState.currentWorkflow?.type === "course_creation") {
          ...
          inferredState.currentWorkflow?.type === "outline_generation"
        ) {
          return await outlineGenerationNode(inferredState);
        } else if (
          inferredState.currentWorkflow?.type === "assignment_creation"
        ) {
          ...
          inferredState.currentWorkflow?.type === "content_generation"
        ) {
 ...
 */
export async function outlineGenerationNode(
  state: ChatbotState,
): Promise<Partial<ChatbotState>> {
  try {
 ...
 */
export async function assignmentCreationNode(
  state: ChatbotState,
): Promise<Partial<ChatbotState>> {
  try {
    // 检查当前工作流步骤
    const currentStep = state.currentWorkflow?.step || "info_collection";
    const existingAssignmentData =
      state.currentWorkflow?.data?.assignmentData || null;
    const selectedClassId =
      state.metadata?.selectedClassId ||
      state.currentWorkflow?.data?.classId ||
      null;
...
💡 提示：您也可以说"查看我的班级列表"来查看可用的班级。`;
      ...
      return {
        ...
        currentWorkflow: {
          ...
          data: {
            assignmentData: {
              title:
                result.assignmentTitle || result.assignmentType || "未命名作业",
    ...
    if (currentStep === "awaiting_class_selection" && existingAssignmentData) {
      ...
      const uuidMatch = userInput.match(
      ...
      // 检查是否用户说"创建到最近的班级"
      const useLatestClass =
        userInput.includes("最近") ||
        userInput.includes("latest") ||
        userInput.includes("最新");
...
不要输出任何其他解释、自然语言前缀/后缀或代码块标记。`;
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      **第${i}次优化改进：**
      ...
      improvements.push(
        `第${i}次迭代：${i === 1 ? "结构优化" : i === 2 ? "内容增强" : "质量提升"}`,
      );
    }
 ...
 * 使用messages格式传递完整对话历史
 */
export async function contentGenerationNode(
  state: ChatbotState,
): Promise<Partial<ChatbotState>> {
  try {
    // 构建完整的对话历史作为messages格式
 ...
 * 不使用任何硬编码关键词检测
 */
async function generalContinueNode(
  state: ChatbotState,
): Promise<Partial<ChatbotState>> {
  try {
    // 构建完整的对话历史作为messages格式
